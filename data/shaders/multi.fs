...
vec3 PhongEquation(in vec3 light_vector, in float light_intensity, in float light_distance, in vec3 normal_vector, in bool light_attenuation)
{
	//Compute vectors
	vec3 L = light_vector;
	...
	//Diffuse factor
	vec3 diffuse_factor = vec3(attenuation_factor * NdotL);
    ...
    //Phong equation
	vec3 light = attenuation_factor *(diffuse_factor + specular_factor) * u_light_color * light_intensity * shadow_factor;
...
{	
	...
	if(u_light_type == 0) //point light
	{
		...
		//Phong Equation
		phong_light += PhongEquation(light_vector,light_intensity,light_distance, normal_vector, true);
	}
	...
	else if(u_light_type == 2) //directional light
	{
		...
		//Phong Equation
		phong_light += PhongEquation(light_vector,light_intensity,light_distance, normal_vector, false);
	}	
